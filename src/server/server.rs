...
use super::{Result, Config, OnResponse};
...
where T: RaftStoreRouter, S: StoreAddrResolver {
...
impl<T: RaftStoreRouter, S: StoreAddrResolver> Server<T, S> {
    ...
    fn on_conn_readable(&mut self, event_loop: &mut EventLoop<Self>, token: Token) -> Result<()> {
        debug!("token: {:?} on_conn_readable", token);
        ...
        if msgs.is_empty() {
            ...
            debug!("token: {:?}  msgs is empty", token);
            return Ok(());
        }

        for msg in msgs {
            debug!("token: {:?}  receive one msg", token);
    ...
    fn new_snapshot_reporter(&self, data: &ConnData) -> SnapshotReporter<T> {
        ...
        SnapshotReporter {
            router: self.ch.raft_router.clone(),
            region_id: region_id,
