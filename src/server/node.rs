...
impl<C> Node<C>
            ...
            where T: Transport + 'static
    {
        ...
        let mut store_id = try!(self.check_store(&db));
        //store没有初始化
        ...
        // inform pd.
        //启动Store
        try!(self.start_store(event_loop, store_id, db, trans, snap_mgr));
        try!(self.pd_client
    ...
    fn check_store(&self, db: &DB) -> Result<u64> {
        let res = try!(db.get_msg::<StoreIdent>(&keys::store_ident_key()));
        if res.is_none() {
    ...
    fn bootstrap_first_region(&self, db: &DB, store_id: u64) -> Result<metapb::Region> {
        ...
        let region = try!(store::bootstrap_region(db, store_id, region_id, peer_id));
        Ok(region)
    }
